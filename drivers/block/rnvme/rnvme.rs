...
impl NvmeDevice {
    ...
    fn configure_admin_queue(
        ...
        let admin_tagset: Arc<mq::TagSet<nvme_mq::AdminQueueOperations>> = Arc::pin_init(
            mq::TagSet::new(1, dev.clone(), queue_depth, 1, bindings::NUMA_NO_NODE),
            flags::GFP_KERNEL,
        )?;
...
impl pci::Driver for NvmeDevice {
    ...
    fn probe(pci_dev: &pci::Device<device::Core>, _id: &Self::IdInfo) -> Result<Pin<KBox<Self>>> {
        ...
        let param_irq_queue_count = *module_parameters::irq_queue_count.get();
        let param_poll_queue_count = *module_parameters::poll_queue_count.get();
        ...
        let device = KBox::pin(Self { data: data }, flags::GFP_KERNEL)?;
